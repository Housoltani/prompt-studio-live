...
export default function LiveGenerator() {
  ...
  const [isSettingsOpen, setIsSettingsOpen] = useState(false); // text, image, video, music
  const [persona, setPersona] = useState(() => getPersonas(t)[0]);
      ...
      <div className="flex gap-6 flex-1 min-h-0">
        ...
        <div className="flex-1 min-h-[600px] glass-card border border-slate-700/50 rounded-3xl flex flex-col overflow-hidden bg-slate-900/80 shadow-2xl relative">
          ...
          <button 
            ...
            className="md:hidden absolute top-4 right-4 z-10 bg-slate-800/90 hover:bg-slate-700 backdrop-blur p-2.5 rounded-lg border border-slate-700 text-slate-300 shadow-lg transition-colors"
          >
        ...
        <div className={`
          ...
          flex flex-col gap-6
        `}>
          <div className="glass-panel p-5 rounded-3xl border border-slate-700/50 shadow-xl bg-slate-900/80 mt-16 md:mt-0">
            <div className="flex justify-between items-center mb-5 border-b border-slate-700/50 pb-3">
              ...
              <button onClick={() => setIsSettingsOpen(false)} className="md:hidden p-1.5 bg-slate-800 rounded-full text-slate-400 hover:text-white">
